...
def generar_interfaz(carga_id):
    ...
    excel_file = st.file_uploader("Archivo Excel (.xlsx)", type=["xlsx"], key=f"excel_{carga_id}")

    # 🖼️ Carga de imágenes con clave dinámica
    st.subheader("🖼️ Paso 2: Cargar evidencias en imagen")
    uploaded_images = st.file_uploader("Imágenes (.png, .jpg)", type=["png", "jpg"], accept_multiple_files=True, key=f"images_{carga_id}")

    # 🔄 Generación de documentos
    if st.button("Generar documentos", key=f"generar_{carga_id}"):
...
generar_interfaz(st.session_state.carga_id)

# 🔄 Botón para nueva carga que reinicia claves
st.markdown("---")
